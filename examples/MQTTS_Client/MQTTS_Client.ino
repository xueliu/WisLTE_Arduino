...
const char mqtt_server[] = "a2t3bs5f5xc3vy.iot.eu-central-1.amazonaws.com ";
unsigned int mqtt_port = 8883;
const char mqtt_client_id[] = "wiscam.000002";
...
void setup(){
    ...
    while(!WisLTE.SetMQTTEnableSSL(comm_mqtt_index, comm_ssl_index, true)){
        DSerial.println("\r\nEnable the SSL Fail!");
        int e_code;
        if (WisLTE.returnErrorCode(e_code)){
            DSerial.print("\r\nERROR CODE: ");
    ...
    while (WisLTE.MQTTSubscribeTopic(comm_mqtt_index, 1, "", mqtt_qos) != 0){
