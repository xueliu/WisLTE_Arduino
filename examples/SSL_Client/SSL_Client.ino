...
const char ssl_ip[] = "www.baidu.com";
const char ssl_cmd[] = "GET / HTTP/1.1\r\n\r\n";
...
WisLTEBG96SSL WisLTE(ATSerial, DSerial);
...
void setup(){
    ...
    DSerial.println(ssl_error);
#endif
    while (!WisLTE.OpenSSLSocket(comm_pdp_index, comm_ssl_index, comm_socket_index, ssl_ip, ssl_port, BUFFER_MODE)){
        ...
        if (WisLTE.returnErrorCode(e_code)){
            ...
            while(1);
        }else if (WisLTE.QueryLastErrorCode(ssl_socket_error)){
    ...
    if (WisLTE.SSLSocketSendData(comm_socket_index, ssl_cmd)){
...
void loop(){
    ...
    unsigned int index;
    char recv_data[128];
    SSL_Socket_Event_t ret = WisLTE.WaitCheckSSLSocketEvent(ssl_event, 2);
    switch(ret)
    {
        case SSL_SOCKET_CLOSE_EVENT:
            index = atoi(ssl_event);
            if(WisLTE.CloseSSLSocket(index)){
                DSerial.println("\r\nClose SSL Socket Success!");
            }
            break;
        case SSL_SOCKET_RECV_EVENT:
            index = atoi(ssl_event);
            if(WisLTE.SSLSocketRecvData(index, 128, recv_data)){
